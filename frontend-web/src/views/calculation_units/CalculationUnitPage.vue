<template>
    <div id="calculation-unit-container">
        <div class="items-center" style="padding: 8px 0 16px 0;">
            ...
            <div class="sub-title">Đơn vị tính</div>
        </div>
        <div class="flex operations">
            <button class="m-btn m-btn-operate align-center" @click="showCalculationUnitAddForm">
        ...
        <div class="m-table-container">
            ...
            <table class="m-table m-table-line">
                ...
                <tbody>
                    <tr :class="{ 'row-active': isActiveRow[index] }" v-for="(unit, index) in units" :key="index"
                        @click="setActive(index)" @dblclick="showFormUpdateCalculationUnit(unit)">
                        <td>{{ unit.calculationUnitName }}</td>
    ...
    <CalculationUnitForm v-if="isShowCalculationUnitform" :unitData="unitSelected" :formDetail="formMode">
    </CalculationUnitForm>
</template>
<script>
...
import CalculationUnitForm from './CalculationUnitForm.vue';
...
export default {
    name: "CalculationUnitPage",
    components: {
        GLoading, GSearchOption, CalculationUnitForm
    },
    created() {
        this.getCalculationUnitsData();
    },
    props: {
        stopfollows: {
    ...
    data() {
        return {
            ...
            isShowCalculationUnitform: false,
        }
    },
    methods: {
        async getCalculationUnitsData() {
            this.isShowLoading = true;
            try {
                let res = await CalculationUnitAPI.getPaging(this.pageSize, this.pageNumber);
                // console.log(res);
                this.units = res.data.listData;
        ...
        showCalculationUnitAddForm() {
            this.formMode = Enum.FormMode.Add;
            this.isShowCalculationUnitform = true;
        },
        showFormUpdateCalculationUnit(unit) {
